...
# Preparación de los datos y modelado de nicho regional de Galeorhinus galeus
...
# Río de la Plata y Lagoa dos Patos
...
# Leer predictors
env <- stack('env.tif')[[1:6]]
var_names <- c('Distance_to_coast', 'Depth', 'Surface_temperature', 'SST_fronts', 'Turbidity', 'Primary_productivity')
names(env) <- var_names
...
legend('bottomright', legend = c('Region de interés', 'Occurrences'),
...
# Leer predictores
env <- stack(paste(season[i], '.tif', sep = ''))[[1:6]]
var_names <- c('Distance_to_coast', 'Depth', 'Surface_temperature', 'SST_fronts', 'Turbidity', 'Primary_productivity')
names(env) <- var_names
