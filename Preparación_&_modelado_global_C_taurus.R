...
# Preparación de los datos y modelado de nicho global de Carcharias taurus
...
#dat <- subset(dat, Region %in% c('Australia', 'Northwest Atlantic'))
...
env.M <- crop(env.G, occ.buff) # cortar el raster al buffer
...
mar_rojo <- matrix(c(31.68, 44.1, 40.15, 28.45, 31.68, 30.91, 26.23, 18.75, 23.01, 30.91), 5, 2)
mar_rojo <- SpatialPolygons(list(Polygons(list(Polygon(mar_rojo)), ID = 1)), proj4string = crs)
...
env.M <- mask(env.M, mar_rojo, inverse = T)
